...
(function () {
  ...
  window.GiftyCloudRender = async function () {
    ...
    grid.querySelectorAll('.cloud-memory-card').forEach(x => x.remove());
    try {
      ...
      memories.forEach(m => {
        ...
        card.onclick = () => {
          ...
          const modalCard=overlay.querySelector('.memory-modal-card'), img=overlay.querySelector('.memory-modal-image'), counter=overlay.querySelector('.memory-modal-counter'), prev=overlay.querySelector('.memory-modal-prev'), next=overlay.querySelector('.memory-modal-nav.memory-modal-next');
          ...
          prev.style.display=next.style.display=imgs.length>1?'block':'none'; prev.onclick=e=>{e.stopPropagation();i=(i-1+imgs.length)%imgs.length;show()}; next.onclick=e=>{e.stopPropagation();i=(i+1)%imgs.length;show()}; overlay.querySelector('.memory-modal-close').onclick=closeViewer; overlay.onclick=e=>{if(e.target===overlay)closeViewer()};
          if (window.GiftyAttachViewerControls) window.GiftyAttachViewerControls(modalCard, m, closeViewer);
        };
